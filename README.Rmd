...
-   The manuals are in the folder `doc/manual`, on `trunk` for R-devel (<https://github.com/wch/r-source/tree/trunk/doc/manual>) and on a release branch/tag for released versions of R. By default this project builds the latest release — see [Building the manuals](#building-the-manuals).
...
By default the build tracks the **latest released** version of R. Set the `R_MANUALS_REF` environment variable to build a different version:
...
The ref refers to a branch or tag in the [`wch/r-source`](https://github.com/wch/r-source) mirror. When `R_MANUALS_REF` is unset, the build detects the newest release tag automatically.
